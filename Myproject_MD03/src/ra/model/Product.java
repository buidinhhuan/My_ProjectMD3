...
public class Product implements Serializable {
    ...
    public String toString() {
        return String.format("%-15s | %-20s | %-10s | %-10s | %-10s | %-10s " ,
                "Mã sản phẩm", "Tên sản phẩm", "Giá", "Mô tả", "Trong kho", "Thương hiệu")
                + "\n" + "-".repeat(98)
                + String.format("\n%-15s | %-20s | %-10s | %-10s | %-10s | %-10s",
                productId, productName, productPrice + " $ ", description, stock +" chai", trademark.getTrademarkName())
                + "\n" + "-".repeat(98)
                + String.format("\nTình trạng : %s", status ? " Đang bán " : " Hết hàng ")
                + "\n" + "—".repeat(98);
    }
